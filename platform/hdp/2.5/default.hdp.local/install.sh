...
hostnamectl --static set-hostname default
...
curl -H "X-Requested-By: ambari" -X POST -d '@/tmp/install/blueprint.json' -u admin:admin http://default.hdp.local:8080/api/v1/blueprints/generated
curl -H "X-Requested-By: ambari" -X POST -d '@/tmp/install/create-cluster.json' -u admin:admin http://default.hdp.local:8080/api/v1/clusters/default
